            ...
            if (s[0] == "!op")
            {
                if (s.Length == 2 && long.TryParse(Identify.Do(s[1]), out _) && 9 <= Identify.Do(s[1]).Length && Identify.Do(s[1]).Length <= 10)
                {
                    if (receiver.Sender.Permission.ToString() == "Owner" || Permission.IsGroupAdmin(receiver.GroupId, receiver.Sender.Id))
                    {
                        switch (Permission.IsGroupAdmin(receiver.GroupId, Identify.Do(s[1])))
                        {
                            case false:
                                ...
                                try
                                {
                                    await receiver.SendMessageAsync($"已将 {Identify.Do(s[1])} 设置为本群机器人管理员");
                                }
                                catch (Exception e)
                                {
                            ...
                            default:
                                ...
                                try
                                {
                                    await receiver.SendMessageAsync($"无法将 {Identify.Do(s[1])} 设置为本群机器人管理员：已经有此或者更高权限");
                                }
                                catch (Exception e)
                                {
                                    Logger.Error("群消息发送失败！");
                    ...
                    else
                    {
                        ...
                        try
                        {
                            await receiver.SendMessageAsync($"无法将 {Identify.Do(s[1])} 设置为本群机器人管理员：权限不足");
                        }
                        catch (Exception e)
                        {
                            Logger.Error("群消息发送失败！");
